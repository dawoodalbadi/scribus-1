...
void ScrPainter::startGraphics(double width, double height)
{
	...
	if (flags & LoadSavePlugin::lfCreateDoc)
	{
		...
		m_Doc->changePageProperties(0, 0, 0, 0, 72 * height, 72 * width, 72 * height, 72 * width, m_Doc->pageOrientation(), m_Doc->pageSize(), m_Doc->currentPage()->pageNr(), 0, m_Doc->pageBiding());
	}
	firstLayer = true;
}
